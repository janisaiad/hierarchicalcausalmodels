...
\State Return the contrast, the fitted factors, and effective-graph diagnostics
\end{algorithmic}
\end{algorithm}
...
Factor diagnostics & $O(Kn)$ summaries & factors independent & approximately $O(Kn/P)$ for factor-wise summaries \\
\bottomrule
\end{tabular}
\end{table}
...
The data are generated from known HSCMs with $n=100$ units and $m=50$ sub-units; recovery of the causal contrast is shown in the convergence plot, so Table~\ref{tab:motifs} instead reports the parallel estimation structure for the same three motifs.
Here $P=4$ denotes four CPU workers and $c$ denotes the average cost of one local density, response, or Monte Carlo evaluation task.
For the GPU column, the local CUDA benchmark was run on an NVIDIA GeForce RTX 4060 Laptop GPU with 8GB VRAM using the torch batched Gaussian HCM kernel with $m=80$ sub-units; the largest successful batch was $2.8\times 10^6$ independent unit tasks in $3.29$ seconds with $6.77$GB peak allocated memory, while $3.0\times 10^6$ tasks exceeded memory.

\begin{table}[!htbp]
\centering
\small
\caption{Parallel estimation structure for the three canonical HCM motifs, including the largest observed single-GPU batch on the local RTX 4060 configuration.}
\label{tab:motifs}
\begin{tabular}{lccc}
\toprule
Motif & Independent tasks & CPU work, $P=4$ & GPU batch ceiling on local config \\
\midrule
Confounder & $n$ response tasks & $\lceil n/4\rceil c$ & up to $2.8\times 10^6$ units \\
Confounder and interference & $2n$ response/mediator tasks & $\lceil 2n/4\rceil c$ & up to $1.4\times 10^6$ units \\
Instrument & $3n$ instrument/weight tasks & $\lceil 3n/4\rceil c$ & up to $9.3\times 10^5$ units \\
\bottomrule
\end{tabular}
\end{table}
...
The same structure is highly GPU-parallelizable, but the GPU analogue of $P$ is not a number of Python workers: units, sub-units, intervention values, and Monte Carlo samples are stacked into tensor batches and evaluated simultaneously when the density estimators are implemented in a tensor backend such as PyTorch or JAX\@.
The STAR timings reported below use CPU parallelism, whereas the local GPU benchmark in Table~\ref{tab:motifs} measures the maximum batch size achievable on the available CUDA device.
...
Beyond the canonical motifs, we use representative benchmark families to check whether the implementation behaves correctly across binary HCMs, spatio-temporal HCMs, and applied synthetic examples with grouped socioeconomic outcomes.
...
The paper reports the cases that are most informative for this scope rather than listing every internal benchmark run.
...
\begin{table}[!htbp]
\centering
\caption{STAR HCM estimates and normalized-factor diagnostic. The Mathematics row is the main outcome analysis; the Reading row is a separate diagnostic run used to inspect the normalized $P(Q^{y\g a,g}\g S)$ factor.}
\label{tab:hcmstar}
\begin{tabular}{llccc}
\toprule
