...
The results show that symbolic identification alone is not enough for practical hierarchical causal inference: scalable estimation, diagnostics, and effective graphs are central parts of the scientific object. \textcolor{red}{mettre en valeur l'utilisation des modèles hierarchiques plutôt que à plat}
}
...
The central device in HCM is the $Q$-variable: a random distribution at the unit level that represents how a sub-unit variable behaves conditional on its sub-unit parents.
...
Section~\ref{sec:method} describes the symbolic-to-numeric backend, Section~\ref{sec:experiments} validates the implementation on controlled settings, Section~\ref{sec:star} gives the Project STAR analysis, and Section~\ref{sec:conclusion} discusses limitations and future work.
...
For instance, one retained collapsed example identifies
\[
  \sum_{Q^{z\g a},W}
  P(W\g Q^a)\,
  P(Y\g Q^a,Q^{z\g a},W)
  ...
  \right].
\]
The corresponding AST can be read horizontally as
\[
...
&
\begin{array}{c}
\texttt{Product}\{P(W\g Q^a),\,P(Y\g Q^a,Q^{z\g a},W)\}\\[2pt]
\texttt{Sum}(Q^a)\{\texttt{Product}[P(Q^{z\g a}\g Q^a,W),\,P(Q^a)]\}
\end{array}
\end{array}.
\]
...
The implementation records such cases and, in the normalized-factor runs studied below, rescales unstable multiplicative factors before evaluation.
...
The data are generated from known HSCMs with $n=100$ units and $m=50$ sub-units, so that the estimated ATE can be compared with a known target rather than only with another estimator.
Table~\ref{tab:motifs} shows that the numerical evaluator recovers the intended causal contrast in these simple settings.

\begin{table}[h!]
\centering
\caption{ATE recovery on canonical HCM motifs.}
\label{tab:motifs}
\begin{tabular}{lccc}
\toprule
Motif & True ATE & Estimated ATE & Absolute error \\
\midrule
Confounder & 2.00 & 1.99 & 0.01 \\
Confounder and interference & 1.50 & 1.52 & 0.02 \\
Instrument & 3.00 & 2.97 & 0.03 \\
\bottomrule
\end{tabular}
\end{table}
...
Because the numerical ATE recovery for the three non-zero motifs is already reported in Table~\ref{tab:motifs}, the gallery is shown graphically rather than repeated as a second numerical table.
Figures~\ref{fig:complete-confounder}--\ref{fig:complete-instrument} display the corresponding complete graph transformations, from the original HCM to the collapsed and augmented or marginalized graphs used by do-calculus.
...
\subsection{Nine benchmark scenarios}

Beyond the canonical examples, the benchmark suite includes nine scenarios grouped into binary HCMs, spatio-temporal HCMs, and real-world-inspired synthetic examples.
These experiments are designed not only to show that the estimator can succeed, but also to identify regimes where the empirical estimand differs from the theoretical target.
Representative results are reported in Table~\ref{tab:benchmarks}.

\begin{table}[h!]
\centering
\caption{Representative benchmark scenarios.}
...
NLSY-style wages & 0.35 log & 0.34 & 0.70 \\
\bottomrule
\end{tabular}
\end{table}

The spatio-temporal experiments are particularly informative because the naive OLS estimate can be five to seven times larger than the true effect when a latent unit variable simultaneously affects baseline outcomes and treatment probability.
...
The experiments also expose a central operational issue: an identified formula may contain multi-parent $Q$-variables whose density factors are numerically unstable.
...
For the HCM analysis we use the kindergarten cohort from this full dataset after removing missing values and balancing the data to 10 students per class, yielding 5,745 students, 322 classes, and 79 schools.
...
School urbanicity ($S_i$) is treated as an observed class-level covariate, while Unobserved class heterogeneity ($U_i$) represents latent class-level variation.
The intervention is therefore not a row-level replacement of $A_{ij}$, but a class-level intervention $\rmdo(Q^a=q_\star^a)$ that fixes the distribution of small-class assignment within the class.
...
\caption{Heavy-tail Q-Q diagnostics for Reading ($Y$) and Mathematics ($M$). The standardized Q-Q plots and GPD tail zooms show departures from a global Gaussian approximation in the score extremes.}
...
\begin{table}[h!]
\centering
\caption{OLS estimates of the small-class effect on kindergarten mathematics.}
\label{tab:ols}
\begin{tabular}{lcccc}
\toprule
...
\end{align}
This model is valuable because it acknowledges between-class heterogeneity, but it treats that heterogeneity as a random effect rather than as a distributional causal mechanism.
...
\]
For the ExactBIC run, we used the score-based DAG stored in the discovery output, with the school-urbanicity/ethnicity relation oriented as Urbanicity ($S$) $\to$ Ethnicity ($E$) for substantive interpretability.
...
After this translation, the class-as-unit HCM v2 model gives the estimates reported in Table~\ref{tab:hcmstar}.
\begin{table}[H]
\centering
\caption{HCM estimates for STAR mathematics outcome.}
\label{tab:hcmstar}
\begin{tabular}{lccc}
\toprule
Run & $E[\rmdo(1)]$ & $E[\rmdo(0)]$ & ATE \\
\midrule
ExactBIC & 2406.65 & 2366.69 & 39.96 \\
ExactBIC (normalized factors) & 1788.10 & 1766.47 & 21.63 \\
\bottomrule
\end{tabular}
\end{table}

These values should not be read as direct replacements for the student-level OLS coefficient.
The OLS coefficient is a student-level contrast for a binary treatment indicator, conditional on observed covariates and school fixed effects.
...
The HCM magnitudes therefore describe the behaviour of the translated hierarchical estimand, and the difference between the raw and normalized-factor rows shows that the result is sensitive to the scaling of multiplicative factors retained in the identified formula.
...
The normalized-factor runs make explicit the difference between a nominal identified functional and the stabilized functional actually evaluated by the numerical code.
For ExactBIC, the normalized term was $P(Q^{y\g a,g}\g S)$, which is the distribution of the class-level Reading ($Y$) mechanism conditional on Small-class assignment ($A$) and Gender ($G$), further conditioned by Urbanicity ($S$).
...
The normalized factor $P(Q^{y\g a,g}\g S)$ is stabilized before the two intervention evaluations, so its raw scale does not dominate the product.
...
The score distributions have heavy, non-Gaussian tails, so Figures~\ref{fig:score-qq-diagnostics} and~\ref{fig:score-hist-diagnostics} report tail diagnostics rather than relying only on a global Gaussian approximation.
This matters for the plug-in analysis because a Gaussian model can describe the central mass of Reading ($Y$) and Mathematics ($M$) while still underrepresenting the probability of extreme classroom scores.
