...
- **Outcome analysé** : `Y` = lecture (`gktreadss`) (intervention toujours sur `Q^a`).
...
- outcome de niveau élève ciblé par ce run : variable `Y` (lecture (`gktreadss`))
...
| `DirectLiNGAM` | `ok` | yes | 17.6184 | 1451.3003 | 1433.6819 |
| `ExactBIC` | `ok` | yes | 21.6281 | 1788.1024 | 1766.4743 |
...
Pour `PC`, `FCI` et `ConsensusMean`, la formule identifiée se réduit souvent à `$P(Q^y)$` (marginalisation sans effet net de `do(Q^a)`). Un `ATE = 0` dans ce cas reflète surtout la structure graphique, pas une preuve d'absence d'effet de la petite classe.
